        ...
        <form action="{{ url($form_action_url) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="id_bank" id="id_bank" class="form-control">
                  ...
                  @foreach ($banks as $v)
                      <option {{ ($data->id_bank == $v->id_bank) ? 'selected' : '' }} value="{{ $v->id_bank }}">{{ $v->nama_bank }}</option>
                  @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="pemilik_rekening" name="pemilik_rekening" value="{{ $data->pemilik_rekening }}"> 
            </div>
            
            <div class="form-group">
                <label for="nomor_rekening">No. Rekening</label>
                <input type="text" class="form-control" id="nomor_rekening" name="nomor_rekening" value="{{ $data->nomor_rekening }}"> 
            </div>
